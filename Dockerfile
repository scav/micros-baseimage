...
ONBUILD RUN  source /tmp/docker.properties \
      ...
      && echo " server /opt/$service_name/config.yml " >> /etc/container_environment/SERVICE_OPTIONS \
      ...
      && if [ $service_version -eq *-SNAPSHOT ] ; then export chosen_repo=$snaptshot_repo_path ; else export chosen_repo=$repo_path ; fi \
      && curl -f -v -o /opt/${service_name}/${service_name}-${service_version}.jar ${repo_path} \
